...
function getColorFromValueAndColors(value, colors, from0To1) {
	...
	for(var i = 0, len = colors.length; i < len - 1; i++) {
		...
		if(value <= ((i + 1) * step + minValue) && value > (i * step + minValue)) {
			return getColorBetween(value, colors[i], colors[i + 1], i * step + minValue, i * (step + 1) + minValue);
		}
	}
	throw value;
...
function getColorBetween(value, color1, color2, color1Val, color2Val) {
	...
	return [parseInt(ratio * Math.abs(color2[0] - color1[0]) + color1[0]), parseInt(ratio * Math.abs(color2[1] - color1[1]) + color1[1]), parseInt(ratio * Math.abs(color2[2] - color1[2]) + color1[2])];
}
